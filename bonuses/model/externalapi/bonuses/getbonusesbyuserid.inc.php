...
*/
class GetBonusesByUserId extends AbstractGet
{
    /**
     * Получает баланс бонусов пользователя по id пользователя
     *
     * @param string $token Авторизационный токен
     * @param integer $id id пользователя в системе ReadyScript
     * @return array
     * @throws ApiException
     * @throws \RS\Exception
     * @example GET /api/methods/bonuses.getBonusesByUserId?token=b45d2bc3e7149959f3ed7e94c1bc56a2984e6a86&id=21196
     *
     * Ответ:
     * <pre>
     *     "response": {
    ...
    protected function process($token, $id)
    {
        if ($admin_user = $this->token->getUser()) {
            ...
            if (empty($user['id'])){
                throw new ApiException(t('Пользователь с данным ID не найден'), ApiException::ERROR_WRITE_ERROR);
            }
