...
class BonusProductDiscountApi
{
     ...
     * @throws \RS\Exception
     */
    function countCartDataTotalWithOutOldPriceItems(Cart $cart, $cart_data)
    {
        $total = 0;
        ...
        foreach ($product_items as $uniq=>$item){
            ...
            if (
                (!$this->isHaveOldCost($product) ||
                !$this->config['disable_use_product_by_action_in_cart'] ||
                $this->shop_config['old_cost_delta_as_discount']) &&
                !$product['bonuses_ignore']
            ){
     ...
     * @throws \RS\Exception
     */
    function checkProductExistsWithMaxDiscount(Cart $cart)
    {
     ...
     */
    function removeDiscountForEachProduct(Cart $cart)
    {
     ...
     * @throws \RS\Exception
     */
    function addOrderDiscountToEachProduct(Cart $cart, $cart_data, $apply_bonuses)
    {
        ...
        foreach ($in_basket as $uniq => $basket_item) {
            ...
            if (!$cart_item->getForbidDiscounts()) {
                if (!empty($this->config['apply_only_for_brands'])){
                    if ($cart_item['type'] == $cart_item::TYPE_PRODUCT){
                        $product = $cart_item->getEntity();
                        if (in_array($product['brand_id'], $this->config['apply_only_for_brands'])){
     ...
     */
    private function addDiscountExtraDataToCartData(Cart $cart, $cart_data, $product_items)
    {
        $new_extra = array();
     ...
     * @throws \RS\Exception
     */
    function removeBeforeSettedDiscount(Cart $cart, $cart_data = false)
    {
        if (!$cart_data){
