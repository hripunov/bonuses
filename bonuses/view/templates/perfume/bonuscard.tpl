...
<div class="bonusCardWrapper personalAccount">
    ...
    {if $bonus_card}
        ...
        <form class="form bonusHistoryAmount balance tocenter" method="POST">
            ...
            {if $bonus_card.active && $bonus_card.amount}
                <input type="submit" name="use_bonus" class="colorButton addFunds" value="Зачислить себе бонусы"/>
            {/if}
        </form>
        </div>
    {else}
        ...
        <div class="buttons cboth">
            <a href="{$router->getUrl('bonuses-front-bonuscard', ['Act'=>'add'])}" class="formSave colorButton inDialog rs-in-dialog">{t}Добавить карту{/t}</a>
        </div>
    {/if}
