...
<div class="bonusCardWrapper">
    {if $bonus_card}
        ...
        {/if}
        <form class="form addMoneyForm" method="POST">
            {if !$bonus_card.active}
                <input type="submit" name="active" class="button color addFunds" value="Активировать"/>
            {/if}
            
            {if $bonus_card.active && $bonus_card.amount}
                <input type="submit" name="use_bonus" class="button color addFunds" value="Зачислить себе бонусы"/>
            {/if}
        </form>
    {else}
        ...
        <div class="buttons cboth">
            <a href="{$router->getUrl('bonuses-front-bonuscard', ['Act'=>'add'])}" class="formSave button color inDialog rs-in-dialog">{t}Добавить карту{/t}</a>
        </div>
    {/if}
</div>