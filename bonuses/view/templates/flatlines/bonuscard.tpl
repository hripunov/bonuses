...
<div class="bonusCardWrapper flatlines personalAccount">
    ...
    {if $bonus_card}
        ...
        <form class="form bonusHistoryAmount balance tocenter" method="POST">
            {if !$bonus_card.active}
                <input type="submit" name="active" class="formSave btn link-more" value="Активировать"/>
            {/if}
            
            {if $bonus_card.active && $bonus_card.amount}
                <input type="submit" name="use_bonus" class="colorButton addFunds btn link-more" value="Зачислить себе бонусы"/>
            {/if}
        </form>
        </div>
    {else}
